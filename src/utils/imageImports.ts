...
const aboutImagesModules = import.meta.glob('../assets/about/*.{png,jpg,jpeg,svg,webp}', { 
  eager: true,
  as: 'url'
});

// Convert to array with metadata for easier use
export const aboutImagesList = Object.entries(aboutImagesModules).map(([path, src]) => {
