...
const getCategoryImages = (category: GalleryCategory) => {
  const featuredImage =
    category.featuredImage?.src || category.images[0]?.src || null;
...
const Home: React.FC = () => {
  const portraitImages = useMemo(
    () => getCategoryImages(portraitGallery),
    [portraitGallery],
  );
  const eventImages = useMemo(
    () => getCategoryImages(eventGallery),
    [eventGallery],
  );
  const concertImages = useMemo(
    () => getCategoryImages(concertGallery),
    [concertGallery],
  );
  const miscellaneousImages = useMemo(
    () => getCategoryImages(miscellaneousGallery),
    [miscellaneousGallery],
  );
  ...
  return (
    <>
      ...
      <div
        id="portfolio"
        className="px-4 md:px-20 lg:px-40 flex flex-1 justify-center py-5"
      >
