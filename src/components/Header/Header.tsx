    ...
    <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#f2f2f2] px-4 md:px-10 py-3">
      ...
      <div className="hidden md:flex flex-1 justify-end gap-8">
        <div className="flex items-center gap-9">
          <Link
            className="text-[#141414] text-sm font-medium leading-normal hover:text-gray-600"
            to="/"
          >
            Home
          </Link>
      ...
      >
        <div className="relative w-6 h-6">
          ...
          <span
            className={`absolute top-0 left-0 w-6 h-0.5 bg-current transform transition-all duration-300 ease-out ${
              isMobileMenuOpen
          ...
          ></span>
          <span
            className={`absolute top-2.5 left-0 w-6 h-0.5 bg-current transform transition-all duration-200 ease-out ${
              isMobileMenuOpen ? "opacity-0 scale-0" : "opacity-100 scale-100"
            }`}
          ></span>
          <span
            className={`absolute top-5 left-0 w-6 h-0.5 bg-current transform transition-all duration-300 ease-out ${
              isMobileMenuOpen
                ? "-rotate-45 -translate-y-2.5"
                : "rotate-0 translate-y-0"
            }`}
      ...
      >
        ...
        {/* Mobile Menu Panel */}
        <div
          className={`absolute top-0 right-0 h-full w-full bg-white transform transition-transform duration-300 ease-out ${
            isMobileMenuOpen && !isClosing
              ? "translate-x-0"
              : "translate-x-full"
          }`}
        >
          ...
          <div className="flex flex-col px-4 py-8">
            ...
            <div
              className={`flex flex-col items-center justify-center mt-12 pt-8 border-t border-[#f2f2f2] transition-all duration-250 ease-out transform ${
                isMobileMenuOpen && !isClosing
                  ? "translate-x-0 opacity-100"
                  : isClosing
                    ? "translate-x-8 opacity-0"
