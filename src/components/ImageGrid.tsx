...
export const ImageGrid: React.FC<ImageGridProps> = ({ className = '' }) => {
  ...
  if (aboutImagesList.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <p className="text-gray-500">No about images found. Please add images to the assets/about folder.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap justify-center gap-4 w-full">
        {aboutImagesList.map((image) => {
          ...
          return (
            ...
            >
              <div className={`${aspectClass} overflow-hidden rounded-xl shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] h-48 sm:h-56 md:h-64 lg:h-72 xl:h-80`}>
                <img 
                  src={image.src}
                  alt={image.alt}
